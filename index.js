...
app.post('/generate-image', async (req, res) => {
  try {
    const { message = 'SPY 1.23-2.13', username = 'trader', channel = 'trading-signals', timestamp = 'Today at 12:00' } = req.body;
    const width = 620;
    const lineHeight = 24;
    const paddingTop = 20;
    ...
    for (const line of messageLines) { ctx.fillText(line, paddingLeft, textY); textY += lineHeight; }
    ...
    const result = await new Promise((resolve, reject) => {
      cloudinary.uploader.upload_stream({ folder: 'discord-trades', resource_type: 'image', format: 'png' }, (error, result) => { if (error) reject(error); else resolve(result); }).end(buffer);
    });
