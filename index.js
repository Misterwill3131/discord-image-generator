...
function downloadUrl(urlStr, maxRedirects) {
            ...
            return new Promise((resolve, reject) => {
                          ...
                          mod.get(urlStr, { headers: { 'User-Agent': 'Mozilla/5.0 Node.js' } }, (res) => {
...
async function loadFonts() {
  ...
  // Load Noto Emoji TTF for emoji rendering
  // Using the official Noto Emoji TTF from Google Fonts GitHub
  const emojiUrls = [
  ...
  for (const emojiUrl of emojiUrls) {
                try {
                                console.log('Trying emoji font:', emojiUrl);
                                const emojiBuf = await downloadUrl(emojiUrl);
                                const tmpDir = os.tmpdir();
                                const ext = emojiUrl.endsWith('.ttf') ? '.ttf' : '.woff2';
                                const emojiPath = path.join(tmpDir, 'NotoEmoji' + ext);
                                ...
                                if (r) {
                                                  ...
                                                  console.log('Emoji font loaded from:', emojiUrl);
                                                  break;
                                }
                } catch (e) {
                                console.error('Emoji font attempt failed for', emojiUrl, ':', e.message);
                }
  }

  if (!emojiFontLoaded) {
                console.log('Emoji font not loaded, emojis will use fallback rendering');
  }
}
...
function drawMixedLine(ctx, text, x, y, emojiSize, FF) {
            ...
            for (const seg of segments) {
                          if (seg.type === 'emoji') {
                                          ...
                                          if (emojiFontLoaded) {
                                                            ctx.font = `${emojiSize}px "${emojiFontFamily}", "${FF}", sans-serif`;
                                          } else {
                                                            ctx.font = `${emojiSize}px "${FF}", sans-serif`;
                                          }
