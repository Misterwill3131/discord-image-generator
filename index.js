...
let emojiFontFamily = 'sans-serif';
...
async function loadFonts() {
  ...
  // Load Noto Emoji font for emoji rendering
  try {
              // Use a smaller subset emoji font from jsDelivr
            const emojiUrl = 'https://cdn.jsdelivr.net/npm/@fontsource/noto-emoji@latest/files/noto-emoji-all-400-normal.woff2';
              const emojiBuf = await downloadUrl(emojiUrl);
              const tmpDir = os.tmpdir();
              const emojiPath = path.join(tmpDir, 'NotoEmoji.woff2');
              fs.writeFileSync(emojiPath, emojiBuf);
              const r = GlobalFonts.registerFromPath(emojiPath, 'NotoEmoji');
              if (r) {
                            emojiFontLoaded = true;
                            emojiFontFamily = 'NotoEmoji';
                            console.log('Emoji font loaded: NotoEmoji');
              }
  } catch (e) {
              console.error('Emoji font error:', e.message);
              // Try fallback: twemoji-colr or another source
            try {
                          // Use Noto Sans Symbols 2 which covers many emoji as fallback
                const sym2Url = 'https://cdn.jsdelivr.net/npm/@fontsource/noto-sans-symbols-2/files/noto-sans-symbols-2-all-400-normal.woff2';
                          const sym2Buf = await downloadUrl(sym2Url);
                          const tmpDir = os.tmpdir();
                          const sym2Path = path.join(tmpDir, 'NotoSansSymbols2.woff2');
                          fs.writeFileSync(sym2Path, sym2Buf);
                          const r = GlobalFonts.registerFromPath(sym2Path, 'NotoSansSymbols2');
                          if (r) {
                                          emojiFontLoaded = true;
                                          emojiFontFamily = 'NotoSansSymbols2';
                                          console.log('Emoji fallback font loaded: NotoSansSymbols2');
                          }
            } catch (e2) {
                          console.error('Emoji fallback font error:', e2.message);
            }
  }
}
...
function drawMixedLine(ctx, text, x, y, emojiSize, FF) {
          ...
          for (const seg of segments) {
                      if (seg.type === 'emoji') {
                                    // Draw emoji using emoji font if available
                        const savedFont = ctx.font;
                                    const savedFill = ctx.fillStyle;
                                    if (emojiFontLoaded) {
                                                    ctx.font = `${emojiSize}px "${emojiFontFamily}", sans-serif`;
                                    } else {
...
app.get('/health', (req, res) => {
          res.json({ status: 'ok', fontLoaded, fontFamily, fontError, avatarLoaded: !!avatarImage, avatarError, emojiFontLoaded, emojiFontFamily });
});
...
app.post('/generate-image', async (req, res) => {
          try {
            ...
            // Timestamp directly after username
